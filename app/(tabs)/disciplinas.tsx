...
import { Plus, BookOpen, Code, Cpu, Calculator, Compass, Atom, Trash2, AlertTriangle, GraduationCap, X, Search } from 'lucide-react-native';
...
export default function DisciplinasScreen() {
  ...
  const filteredDisciplines = disciplines.filter(d => {
    ...
    if (selectedPeriod !== 'all') {
      const isOpt = selectedPeriod === 0;
      const dOpt = d.period === 0 || d.period === null || d.period === undefined;
      matchPeriod = isOpt ? dOpt : d.period === selectedPeriod;
    }
    
    let matchSearch = true;
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      matchSearch = d.name.toLowerCase().includes(q) || Boolean(d.code?.toLowerCase().includes(q));
    }
    
    return matchPeriod && matchSearch;
  });

  const groupedDisciplines = filteredDisciplines.reduce((acc, curr) => {
    const p = curr.period || 0;
    if (!acc[p]) acc[p] = [];
    acc[p].push(curr);
    return acc;
  }, {} as Record<number, typeof disciplines>);

  const handleDelete = (id: string, name: string) => {
    const tCount = tasks.filter(t => t.disciplineId === id).length;
    const sCount = schedules.filter(s => s.disciplineId === id).length;
    const eCount = exams.filter(e => e.disciplineId === id).length;
    const cascadeMsg = (tCount || sCount || eCount) 
      ? `\n\nIsso também removerá ${sCount} horário(s), ${tCount} tarefa(s) e ${eCount} prova(s).` 
      : '';

    Alert.alert(
      'Remover Matéria',
      `Deseja realmente remover a matéria "${name}"?${cascadeMsg}`,
      [
  ...
  const renderDisciplineCard = (subject: Discipline, idx: number) => {
    const period = subject.period || 0;
    const matteColor = subject.color || MATTE_COLORS[period % MATTE_COLORS.length];
    ...
    return (
      <View key={subject.id} style={[styles.squareCard, { backgroundColor: matteColor }]}>
        ...
        <View style={styles.gradesRow}>
          ...
          <Text style={[styles.gradesStatus, { color: subject.statusColor || 'rgba(0,0,0,0.6)' }]}>
          ...
          >
            <Text style={[styles.addGradeBtnText, { color: cardTextColor }]}>Editar Docente / Notas</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
      ...
      >
        ...
        {filteredDisciplines.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Tente mudar os filtros de busca ou período.</Text>
            <TouchableOpacity style={styles.emptyBtn} onPress={() => router.push('/nova-disciplina')}>
              <Plus size={16} color="#FFFFFF" />
              <Text style={styles.emptyBtnText}>Cadastrar Matéria</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <View>
            {selectedPeriod === 'all' && !searchQuery ? (
              ...
              Object.keys(groupedDisciplines)
                .map(Number)
                .sort((a, b) => a - b)
                .map((periodKey) => (
                  <View key={`period-${periodKey}`} style={styles.groupContainer}>
                    <Text style={styles.groupTitle}>
                      {periodKey === 0 ? 'Optativas' : `${periodKey}º Período`}
                    </Text>
                    <View style={styles.gridContainer}>
                      {groupedDisciplines[periodKey].map((subject, idx) => renderDisciplineCard(subject, idx))}
                    </View>
                  </View>
                ))
            ) : (
              // Lista plana
              <View style={styles.gridContainer}>
                {filteredDisciplines.map((subject, idx) => renderDisciplineCard(subject, idx))}
              </View>
            )}
          </View>
        )}
...
function makeStyles(colors: ReturnType<typeof useTheme>['colors'], isDark: boolean) {
  return StyleSheet.create({
    ...
    codeText: { fontFamily: FONTS.medium, fontSize: 9, textTransform: 'uppercase', letterSpacing: 0.5, opacity: 0.6 },
    subjectTitle: { fontFamily: FONTS.bold, fontSize: 13, lineHeight: 16, marginTop: 2 },
    ...
    addGradeBtnText: { fontFamily: FONTS.semiBold, fontSize: 9, opacity: 0.8 },
